...
\noindent\textbf{Mengapa blok ini?} Berkas \texttt{requirements.txt} mencatat paket Python dan batas versi minimum agar rekan atau asisten dapat mereplikasi lingkungan dengan \texttt{pip install -r}; baris yang diawali \texttt{\#} adalah komentar di format pip (bukan Python).
...
\noindent\textbf{Mengapa blok ini?} Mencetak \texttt{\_\_version\_\_} dari pustaka utama membantu men-debug ketidakcocokan API antar versi saat peer review menjalankan notebook Anda.
...
\noindent\textbf{Mengapa blok ini?} Perintah \texttt{pip} dijalankan di terminal/shell sistem operasi (bukan di sel kode Python kecuali Anda sengaja memakai \texttt{!pip} di Jupyter).
