...
\MulaiPenjelasanKode Dataset kanker payudara adalah klasifikasi biner. \path{classification_report} merangkum precision, recall, dan F1. \path{RocCurveDisplay.from_estimator} menggambarkan TPR terhadap FPR dari skor \path{decision_function} regresi logistik. \path{f1_score(..., average="macro")} merata-ratakan F1 antar kelas tanpa membobot frekuensi kelas. \SelesaiPenjelasanKode
...
\MulaiPenjelasanKode \texttt{precision\_recall\_fscore\_support} mengembalikan array per kelas Iris; \texttt{zip} menggabungkan metrik dengan \texttt{support} (jumlah sampel uji per kelas). \SelesaiPenjelasanKode
