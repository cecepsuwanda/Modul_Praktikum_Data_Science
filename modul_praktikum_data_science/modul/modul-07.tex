...
\noindent\textbf{Mengapa blok ini?} \texttt{LinearRegression} meminimalkan MSE ordinari; metrik \texttt{MAE} dan \texttt{RMSE} mengukur error pada \textbf{data uji} setelah model di-\texttt{fit} pada train.
...
\noindent\textbf{Mengapa blok ini?} Baseline memprediksi konstanta = rata-rata target train; RMSE baseline menunjukkan apakah model linier memberi peningkatan bermakna.
...
\noindent\textbf{Mengapa blok ini?} \path{Ridge(alpha=...)} menambah penalti L2 pada koefisien; \path{StandardScaler} di pipeline menstandarkan fitur agar besaran penalti seimbang antar fitur.
