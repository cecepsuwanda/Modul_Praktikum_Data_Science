...
\noindent\textbf{Mengapa blok ini?} \path{cross_val_score} menjalankan $k$-lipat validasi silang: estimator \path{clf} di-\path{clone} dan dievaluasi pada lipatan berbeda; hasilnya adalah array skor lipatan (di sini akurasi)---pemanggilan \path{mean()} memberi estimasi tunggal kinerja sebelum menyentuh \path{X_test}.
...
\noindent\textbf{Mengapa blok ini?} \path{GridSearchCV} mencoba semua kombinasi hiperparameter di \path{grid} pada setiap lipatan CV; \path{best_params_} memilih yang terbaik di train, lalu \path{score} pada \path{X_test} adalah \textbf{satu} estimasi generalisasi (jangan ulangi pencarian dengan melihat test berkali-kali).
...
\noindent\textbf{Mengapa blok ini?} \path{cross_validate} mengembalikan kamus berisi skor train/test per lipatan untuk beberapa metrik sekaligus; di sini kita ambil rata-rata akurasi dan F1-macro di lipatan test.
