...
K3 mendukung pembelajaran berkelanjutan: peralatan yang dirawat tetap andal untuk praktikan berikutnya. Etika akademik menjaga kepercayaan pada bukti capaian Anda---plagiarisme notebook setara dengan plagiarisme laporan tertulis.
...
\noindent\textbf{Mengapa blok ini?} Blok berikut memverifikasi bahwa kernel notebook memakai interpreter Python yang Anda duga. Modul \texttt{sys} menyediakan \texttt{sys.version} (string versi) dan \texttt{sys.executable} (path berkas \texttt{python}); keduanya sering dipaketkan dalam sel pertama saat debugging lingkungan atau saat dokumentasi tugas.
...
\noindent\textbf{Mengapa blok ini?} Contoh ini membedakan \textbf{komentar Python} (baris dengan \texttt{\#}, tidak dieksekusi sebagai perintah) dari \textbf{teks narasi} yang ditulis di sel Markdown. Variabel \texttt{nama\_kampus} bertipe \texttt{str}, \texttt{semester} bertipe \texttt{int}; \texttt{f"..."} adalah \textit{f-string} untuk menyisipkan nilai variabel ke dalam satu string cetak.
...
\noindent\textbf{Mengapa blok ini?} Kamus \texttt{Sumber\_data} menyimpan metadata etika (URL, lisensi, tanggal, catatan) dalam struktur yang mudah disalin ke laporan. \texttt{json.dumps} mengubah kamus menjadi teks JSON; argumen \texttt{indent=2} merapikan baris, \texttt{ensure\_ascii=False} mempertahankan aksara non-ASCII (mis.\ bahasa Indonesia).
